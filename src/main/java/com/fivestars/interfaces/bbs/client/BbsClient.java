 ...
 */
public class BbsClient extends PHPFunctions {
	...
	private static String UC_IP = "127.0.0.1";
	...
	public String uc_user_edit(Map<String, Object> user, String $newpw) {
		String $username = (String) user.get("username");
		String $email = (String) user.get("email");
		String $oldpw = (String) user.get("password");
		return uc_user_edit($username, $oldpw, $newpw, $email, 1, "", "");
	}
	/**
	 * 编辑用户
	 *
	...
	public String uc_user_edit(String $username, String $oldpw, String $newpw,
		...
		return call_user_func("user", "edit", args);
	}
		...
		for (String $k : $arg.keySet()) {
			...
			if ($v.getClass().isAssignableFrom(Map.class)) {
				...
				for (String $k2 : ((Map<String, Object>) $v).keySet()) {
					...
					$k2 = urlencode($k2);
					$s2 += $sep2 + "{" + $k + "}[" + $k2 + "]="
							+ urlencode(String.valueOf($v2));
					$sep2 = "&";
				}
				$s.append($sep).append($s2);
			} else {
				$s.append($sep).append($k).append("=")
						.append(urlencode(String.valueOf($v)));
			}
			$sep = "&";
		}
	...
	public String uc_authcode(String $string, String $operation, String $key,
		...
		String $keyc = $ckey_length > 0 ? ($operation.equals("DECODE")
	...
	protected String uc_fopen(String $url, int $limit, String $post,
		...
		Socket $fp = null;
		try {
			$fp = new Socket($ip != null && $ip.length() > 10 ? $ip : $host,
					$port);
			if (!$fp.isConnected())
				return "";
	...
	public String uc_user_register(String $username, String $password,
		...
		args.put("questionid", $questionid);
		args.put("answer", $answer);
		return call_user_func("user", "register", args);
	}
	...
	public String uc_user_login(String $username, String $password, int $isuid,
		...
		args.put("questionid", $questionid);
		args.put("answer", $answer);
		String $return = call_user_func("user", "login", args);
		return $return;
	}
	...
	public String uc_get_user(String $username, int $isuid) {
		...
		args.put("username", $username);
		args.put("isuid", $isuid);
		String $return = call_user_func("user", "get_user", args);
		return $return;
	}
