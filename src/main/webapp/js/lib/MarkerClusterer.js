...
(function(){
    ...
    var MarkerClusterer =  
        ...
        BMapLib.MarkerClusterer = function(map, options){
            ...
            this._markers = [];
            this._clusters = [];
            this._invisibleMarkers = []; //已经聚合不可见的标记  add by zhuyulei
            var opts = options || {};
            ...
            this._map.addEventListener("zoomend",function(){
            	if(!laso.page.bmap.moveFlag){
            		that._redraw();
            	}
                
            	laso.page.bmap.moveFlag = false;
                //add by wangb 为了缩放后仍能显示infownd
                /*if(laso.page.bmap.focusObjId != null){
                	setTimeout(laso.page.bmap.showMarkerInfoWindow4Cluster(),1000);
                }*/
            });
    ...
    MarkerClusterer.prototype._createClusters = function(){
    	this._invisibleMarkers.length = 0;//add by zhuyulei 
    	this._invisibleMarkers = [];//add by zhuyulei 
        ...
        //add by zhuyulei 解决当地图移动、放大缩小时，marker的label消失的问题
        for(var i = 0, marker; marker = this._markers[i]; i++){
        	var index = indexOf(marker, this._invisibleMarkers);
	        if(index === -1){//marker不在隐藏的cluster中
        		//if(marker.getLabel()){
	        	 var label = new BMap.Label(marker.getTitle(),{offset:new BMap.Size(-20,-20)});
	    		 label.setStyle({backgroundColor:"#2894FF", color:"#fff", border:0, padding:"3px 10px"});
	    		 if(laso.page.base.isShowLpno){
    ...
    MarkerClusterer.prototype._addToClosestCluster = function (marker){
        ...
        if (clusterToAddTo && clusterToAddTo.isMarkerInClusterBounds(marker)){
            ...
            //add by zhuyulei 
            var markers = clusterToAddTo._markers;
            var len = markers.length;
            if(len > 0){
            	for(var i = 0; i < len; i++){
            		var index = indexOf(markers[i], this._invisibleMarkers);
			        if(index === -1){
	            		this._invisibleMarkers.push(markers[i]);
			        }
            	}
            }
            //add end
    ...
    /** add by zhuyulei
     * 获取隐藏的标记。
     * @return {Array<InvisibleMarkers>} 隐藏的标记数组。
     */
    MarkerClusterer.prototype.getInvisibleMarkers = function() {
        return this._invisibleMarkers;
    };
     ...
     * @return {null}
     */
    MarkerClusterer.prototype.updateClusterAndMarker = function(){
    	if (this._map.getZoom() > this._maxZoom) {
    		if(this._clusters.length > 0){
    			for(var i=0; i<this._clusters.length; i++ ){
    				this._map.removeOverlay(this._clusters[i]._clusterMarker);
    			}
    		}
            ...
            for (var i = 0, marker; marker = this._markers[i]; i++) {
	    		...
	    		if(laso.page.bmap.focusObjId != null && laso.page.bmap.focusObjId == marker.objId){
	    			setTimeout("laso.page.bmap.showMarkerInfoWindow4Cluster()",500);
                 }
            }
    ...
    function Cluster(markerClusterer){
        ...
        this._invisibleMarkers = markerClusterer._invisibleMarkers;
    ...
    Cluster.prototype.remove = function(){
        for (var i = 0, m; m = this._markers[i]; i++) {
        	//modify by zhuyulei
                //this._markers[i].getMap() && this._map.removeOverlay(this._markers[i]);
	            m.getMap() && this._map.removeOverlay(m);
		    //modify end
        }//清除散的标记点
